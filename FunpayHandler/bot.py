...
class FunpayBot:
    ...
    def _generate_ticket_comment(self, order_id: str, buyer: str, lot_number: int | None) -> str:
        ...
        prompt = (
            "Сформулируй вежливое обращение в поддержку FunPay (адресат — сотрудник поддержки, не покупатель). "
            "Сообщи, что заказ выполнен, покупатель получил данные, но не подтвердил заказ. "
            "Попроси поддержку подтвердить выполнение. "
            f"Заказ: {order_id}. Покупатель: {buyer}." + (f" Лот №{lot_number}." if lot_number else "")
        )
        fallback = (
            f"Здравствуйте, поддержка FunPay! Заказ {order_id} выполнен, покупатель {buyer} "
            f"не подтвердил выполнение. Просьба подтвердить заказ. Спасибо!"
        )
        if not api_key:
            return fallback
        try:
